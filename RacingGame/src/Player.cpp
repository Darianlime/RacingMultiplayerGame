...
void Player::Update(float dt) {
	...
	car.velocity -= car.velocity * dt; // friction / coasting
	//printf("Velocity: %.2f\n", velocity);
	float turnRate = 100.0f;
	float calcRot = glm::sign(car.velocity) * turnRate * dt;
	float oversteerAngle = 0.0f;
	if (car.velocity > 1000.0f && (((Keyboard::key(GLFW_KEY_SPACE) && Keyboard::key(GLFW_KEY_D))) || ((Keyboard::key(GLFW_KEY_SPACE) && Keyboard::key(GLFW_KEY_A))))) {
